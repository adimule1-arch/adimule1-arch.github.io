...
def generate_blog_html(blog_metadata, blog_content, output_directory=DEFAULT_BLOGS_DIR):
    ...
    try:
        ...
        permalink = blog_metadata.get("permalink", f"/blogs/{blog_id}.html")
        thumbnail = blog_metadata.get("thumbnail", "content/site/profile_picture.jpg")

        # Ensure thumbnail and canonical post URL are absolute URLs
        site_url = get_site_url()
        if not thumbnail.startswith("http"):
            thumbnail = f"{site_url}/{thumbnail}" if site_url else thumbnail
        post_url = permalink if permalink.startswith("http") else f"{site_url}{permalink}"
...
def convert_markdown_to_json(markdown_file_path, output_directory=None):
    ...
    post_permalink = frontmatter_data.get("permalink", "")
    ...
    blog_metadata = {
        ...
        "permalink": post_permalink or f"/blogs/{unique_blog_id}.html",
